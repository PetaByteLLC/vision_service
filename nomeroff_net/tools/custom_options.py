...
class CustomOptionsMaker:
    ...
    def __init__(
        ...
        self.dirpath_custom_rebalance = "{}-{}".format(
            self.dirpath_custom, rebalance_suffix
        )

        if dataset_count_line_classes is not None:
            _, self.count_line_converter_all_idx = self.make_convertor(
                dataset_count_line_classes
            )
        if custom_count_line_classes is not None:
            self.count_line_converter_custom, _ = self.make_convertor(
                custom_count_line_classes
            )

        if dataset_region_classes is not None:
            _, self.region_converter_all_idx = self.make_convertor(
                dataset_region_classes
            )
    ...
    def make_convertor(class_region: List) -> Tuple:
        ...
        for i, class_name in enumerate(
            class_region
        ):  # OptionsDetector.CLASS_REGION_ALL
    ...
    def filter_custom_dataset(self, option_dir: str, verbose: bool = False) -> None:
        ...
        ann_dir = os.path.join(
            self.dirpath, option_dir, self.custom_options_sub_dirs[0]
        )
        ...
        for dir_name, subdir_list, file_list in os.walk(ann_dir):
            for fname in file_list:
                ...
                if self.filter_by_state_id(json_data):
                    cnt += self.make_custom_record(
                        option_dir, copy.deepcopy(json_data), verbose
                    )
                else:
                    filtered_cnt += 1
    ...
    def filter_by_state_id(self, json_data):
        if len(self.state_ids):
            return (
                "state_id" in json_data and int(json_data["state_id"]) in self.state_ids
            )
        else:
            return True

    def make_custom_record(
        self, option_dir: str, json_data: Dict, verbose: bool = False
    ) -> int:
        ann_file_to = os.path.join(
            self.dirpath_custom,
            option_dir,
        ...
        if (
            self.count_line_converter_all_idx is not None
            and self.count_line_converter_custom is not None
        ):
            custom_count_line_name = self.count_line_converter_all_idx[
                int(json_data["count_lines"])
            ]
            if custom_count_line_name not in self.count_line_converter_custom:
                return 0
            all_count_line_id = json_data["count_lines"]
            custom_count_line_id = self.count_line_converter_custom[
                custom_count_line_name
            ]
            json_data["count_lines"] = custom_count_line_id
            if verbose:
                print(
                    "{} {} -> custom_count_line_id {} -> {}".format(
                        json_data["name"],
                        custom_count_line_name,
                        all_count_line_id,
                        custom_count_line_id,
                    )
                )
        if (
            self.region_converter_all_idx is not None
            and self.region_converter_custom is not None
        ):
            custom_region_name = self.region_converter_all_idx[
                int(json_data["region_id"])
            ]
    ...
    def get_regions_stats(self, custom_options_dir):
        ann_dir = os.path.join(
            self.dirpath_custom, custom_options_dir, self.custom_options_sub_dirs[0]
        )
        options_stats = {}
        for dir_name, subdir_list, file_list in os.walk(ann_dir):
            for fname in file_list:
    ...
    def duplicate_class_items(
        ...
        dirpath_custom_options_dir = os.path.join(
            self.dirpath_custom, custom_options_dir
        )
        dirpath_custom_options_dir_ann = os.path.join(
            dirpath_custom_options_dir, self.custom_options_sub_dirs[0]
        )
        dirpath_custom_options_dir_img = os.path.join(
            dirpath_custom_options_dir, self.custom_options_sub_dirs[1]
        )
        ...
        if self.verbose:
            print(
                "Try make duplicate/augmentation for {} items".format(
    ...
    def move_unused_items_to_rebalance_dir(
        self,
        ...
        dirpath_custom_rebalance_options_dir = os.path.join(
            self.dirpath_custom_rebalance, custom_options_dir
        )
        dirpath_custom_rebalance_options_ann = os.path.join(
            dirpath_custom_rebalance_options_dir, self.custom_options_sub_dirs[0]
        )
        dirpath_custom_rebalance_options_img = os.path.join(
            dirpath_custom_rebalance_options_dir, self.custom_options_sub_dirs[1]
        )
    ...
    def add_class_entries(
        self, options_stat: Dict, custom_options_dir: str, with_aug: bool = False
    ):
        ...
        if self.verbose:
            print(
                "Multiply {} class data in {} times and add random {} items of class".format(
                    region_cnt, copies_cnt, modulo
                )
            )
        ...
        if self.verbose:
            print(
                "Add appendix ({} items) for index {}".format(
                    len(options_stat_appendix.keys()), copies_cnt
                )
            )
        self.duplicate_class_items(
            options_stat_appendix, copies_cnt, custom_options_dir, with_aug
        )
        return 1

    def rebalance_region(
        self,
        rebalance_options_stats: Dict,
        region_id: int,
        custom_options_dir: str,
        ...
        if items_per_class < region_cnt:
            balanced_region_items = random.sample(
                list(options_stat.keys()), items_per_class
            )
            if self.verbose:
                print(
                    "Crop class region_id {} to {}".format(
                        region_id, len(balanced_region_items)
                    )
                )
            self.move_unused_items_to_rebalance_dir(
                rebalance_options_stats,
                region_id,
            ...
            if self.verbose:
                print(
                    "Increase class region_id {} from {} to {}".format(
                        region_id, len(options_stat.keys()), items_per_class
                    )
                )
            self.add_class_entries(options_stat, custom_options_dir, with_aug)

    def rebalance_count_lines_item(
        self,
        rebalance_options_stats: Dict,
        ...
        if items_per_class < count_lines_cnt:
            ...
            if self.verbose:
                print(
                    "Crop class count_lines {} to {}".format(
                        count_lines, len(balanced_region_items)
                    )
                )
            self.move_unused_items_to_rebalance_dir(
                rebalance_options_stats,
            ...
            if self.verbose:
                print(
                    "Increase class region_id {} from {} to {}".format(
                        count_lines, len(options_stat.keys()), items_per_class
                    )
                )
            self.add_class_entries(options_stat, custom_options_dir, with_aug)

    def rebalance_regions(
        self,
        ...
        if os.path.exists(self.dirpath_custom):
            ...
            if not os.path.exists(dirpath_custom_rebalance_options_dir):
                for sub_dir in self.custom_options_sub_dirs:
                    os.makedirs(
                        os.path.join(dirpath_custom_rebalance_options_dir, sub_dir),
                        exist_ok=True,
                    )
                ...
                for region_id in rebalance_options_stats:
                    ...
                    self.rebalance_region(
                        rebalance_options_stats, region_id, custom_options_dir, with_aug
                    )
            else:
                print("Rebalancing is possible only once!")
        else:
            print(
                'Rebalancing is possible only after calling the make method for custom dir: "{}"!'.format(
                    self.dirpath_custom
                )
            )
        return 1

    def rebalance_count_lines(
        self,
        custom_options_dir: str = "train",
        with_aug: bool = False,
        verbose: bool = False,
        ...
        if os.path.exists(self.dirpath_custom):
            ...
            if not os.path.exists(dirpath_custom_rebalance_options_dir):
                ...
                for count_lines in rebalance_options_stats:
                    ...
                    self.rebalance_count_lines_item(
                        rebalance_options_stats,
                        count_lines,
                        custom_options_dir,
                        with_aug,
