...
def define_left_and_right_zones(lines_arr, zones):
    ...
    if len(lines_arr) == 1:
        ...
        left_zones, right_zones = split_zones(
            [zones[line["idx"]] for line in lines_arr]
        )
    if len(lines_arr) == 2:
...
def prepare_multiline_rects(rects, zones, lines):
    ...
    for idx in lines.keys():
        ...
        if idx == 1 and lines_count == 2:
            left_zone, right_zone, left_zones, right_zones = (
                define_left_and_right_zones(lines_arr, zones)
            )
            new_zones = add_to_zones_left_zones(new_zones, left_zone, left_zones)
        else:
            for line in lines[idx]:
                new_zones.append(zones[line["idx"]])
    new_zones = add_to_zones_right_zones(
        lines_count, new_zones, right_zone, right_zones
    )
