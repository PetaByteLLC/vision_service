...
from nomeroff_net.data_loaders.fraud_numberplate_generator import (
    FraudImgGenerator as ImgGenerator,
)
    ...
    def __init__(
        ...
        if train_dir is not None:
            self.train_image_generator = data_loader(
                train_dir, width, height, batch_size
            )
        ...
        if val_dir is not None:
            self.val_image_generator = data_loader(val_dir, width, height, batch_size)

        # init test generator
        self.test = None
        self.test_image_generator = None
        if test_dir is not None:
            self.test_image_generator = data_loader(test_dir, width, height, batch_size)
