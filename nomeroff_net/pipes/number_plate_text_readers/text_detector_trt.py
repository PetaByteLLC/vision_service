...
class TextDetectorTrt(TextDetector):
    def __init__(
        ...
        TextDetector.__init__(
            self, presets, default_label, default_lines_count, load_models=False
        )
        for i, detector_class in enumerate(self.detectors):
            trt_detector_class = type(
                f"{detector_class.get_classname()}_trt",
    ...
    def get_static_module(name: str) -> object:
        ...
        trt_detector_class = type(
            f"{detector.__class__.get_classname()}_trt",
