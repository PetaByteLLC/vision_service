...
def mean_rect_distance(rect: List, start_idx: int) -> np.ndarray:
    ...
    return np.mean(
        [distance(rect[idxs[0]], rect[idxs[1]]), distance(rect[idxs[2]], rect[idxs[3]])]
    )
...
class MultilineConverter:
        ...
        if zones_cnt > 1:
            for idx in range(1, len(self.rects)):
                ...
                lines[current_line].append(
                    {"idx": idx, "h": h, "y": y, "x": x, "matrix": matrix}
                )
        return lines
    ...
    def merge_lines(self, cmd: Any) -> np.ndarray:
        ...
        img_zones, np_config = cmd.prepare_multiline_rects(
            self.rects, img_zones, self.probably_lines
        )

        if np_config is None:
            np_config = {}
        padding_left_coef = np_config.get(
            "padding-left-coef", np_config_default["padding-left-coef"]
        )
        padding_right_coef = np_config.get(
            "padding-right-coef", np_config_default["padding-right-coef"]
        )
        padding_zones_coef = np_config.get(
            "padding-zones-coef", np_config_default["padding-zones-coef"]
        )
        padding_top_coef = np_config.get(
            "padding-top-coef", np_config_default["padding-top-coef"]
        )
        padding_bottom_coef = np_config.get(
            "padding-bottom-coef", np_config_default["padding-bottom-coef"]
        )
        ...
        res_zone = [
            np.ones(
                (target_zone_value, int(target_zone_value / padding_left_coef)),
                dtype="uint8",
            )
            * self.bg_fill,
            temp,
            np.ones(
                (target_zone_value, int(target_zone_value / padding_right_coef)),
                dtype="uint8",
            )
            * self.bg_fill,
        ]
        ...
        res_zone = [
            np.ones(
                (int(target_zone_value / padding_top_coef), temp.shape[1]),
                dtype="uint8",
            )
            * self.bg_fill,
            temp,
            np.ones(
                (int(target_zone_value / padding_bottom_coef), temp.shape[1]),
                dtype="uint8",
            )
            * self.bg_fill,
        ]
