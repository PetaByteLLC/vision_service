...
from nomeroff_net.tools import (
    fline,
    distance,
...
from nomeroff_net.pipes.number_plate_multiline_extractors.bbox_np_multiline import (
    MultilineConverter,
)
...
def fix_mline_boxes_angle(mline_boxes: List) -> Tuple[List, float]:
    ...
    for mline_idx, mline_box in enumerate(mline_boxes):
        if abs(target_angle - options[mline_idx]["angle"]) > max_delta_angle:
            mline_boxes[mline_idx] = fix_box_angle(
                target_angle, mline_box, options[mline_idx]
            )
...
def apply_new_box_angle(box: List, dx: float, dy: float) -> List[np.ndarray]:
    ...
    left_matrix = linear_line_matrix(
        left_reference_point, apply_new_point(left_reference_point, dx, dy)
    )
    right_matrix = linear_line_matrix(
        right_reference_point, apply_new_point(right_reference_point, dx, dy)
    )
...
def build_new_points(
    left_reference_point: List, right_reference_point: List, dx: float, dy: float
) -> List[List]:
    return [
...
def fit_to_frame(
    target_points: np.ndarray or List, mline_boxes: List, shape: Tuple
) -> Tuple[List, List]:
    """
    Неработающая заглушка
    ...
    h = shape[0]
    w = shape[1]
    out_of_bounds_points, out_of_bounds_points_idx = check_line_side_faces(
        target_points, shape
    )
    if len(out_of_bounds_points):
        ...
        if p2_dx > p0_dx:
            ...
            dx = -dx
            dy = -dy
        target_points = build_new_points(
            left_reference_point, right_reference_point, dx, dy
        )
        mline_boxes = [
            apply_new_box_angle(mline_box, dx, dy) for mline_box in mline_boxes
        ]
...
class CCraft(object):
    def multiline_to_one_line(
        ...
        res = [
            self.make_one_line_from_many(
                mline_boxes, image_part, region_name, multiply_coef
            )
            for mline_boxes, image_part, region_name in zip(
                all_mline_boxes_rect, image_parts, region_names
            )
        ]
        return (
    ...
    def make_one_line_from_many(
        ...
        if len(mline_boxes) > 1:
            ...
            target_points = minimum_bounding_rectangle(
                np.concatenate(mline_boxes, axis=0)
            )
            target_points = normalize_multiline_rect(target_points, mline_boxes)
            target_points, mline_boxes = fit_to_frame(
                target_points, mline_boxes, image_part.shape
            )
            multiline_converter = MultilineConverter(
                image_part, mline_boxes, target_points
            )
...
def convert_multiline_to_one_line(
    img,
    zones,
    all_mline_boxes,
    target_boxes,
    ...
    all_mline_boxes_rect = [
        mline_boxes for mline_boxes, cl in zip(all_mline_boxes, count_lines) if cl > 1
    ]
    region_names_rect = [
        region_name for region_name, cl in zip(region_names, count_lines) if cl > 1
    ]
    index_rect = [i for i, cl in enumerate(count_lines) if cl > 1]
    (zones_rect, zones_target_points, zones_mline_boxes) = ccraft.multiline_to_one_line(
        all_mline_boxes_rect, image_parts, region_names_rect
    )
...
def convert_multiline_images_to_one_line(
    image_ids,
    images,
    zones,
    images_mline_boxes,
    ...
    for image_id, (image, image_mline_boxes, image_bboxs) in enumerate(
        zip(images, images_mline_boxes, images_bboxs)
    ):
        image_zones = [zone for _id, zone in zip(image_ids, zones) if _id == image_id]
        image_count_lines = [
            count_line
            for _id, count_line in zip(image_ids, count_lines)
            if _id == image_id
        ]
        image_region_names = [
            region_name
            for _id, region_name in zip(image_ids, region_names)
            if _id == image_id
        ]
        zone_ids = [zone_id for zone_id, _id in enumerate(image_ids) if _id == image_id]
        new_zones = convert_multiline_to_one_line(
            image,
            image_zones,
            image_mline_boxes,
            image_bboxs,
