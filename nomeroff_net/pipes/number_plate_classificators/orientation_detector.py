...
from nomeroff_net.data_modules.numberplate_orientation_data_module import (
...
class OrientationDetector(object):
    ...
    def prepare(
        ...
        if train_json_path is None:
            train_json_path = os.path.join(
                base_dir, "train/via_region_data_orientation.json"
            )
        validation_dir = os.path.join(base_dir, "val")
        if validation_json_path is None:
            validation_json_path = os.path.join(
                base_dir, "val/via_region_data_orientation.json"
            )
    ...
    def load(
        ...
        if path_to_model == "latest":
            ...
            model_info = modelhub.download_model_by_url(
                path_to_model, self.get_classname(), "numberplate_orientation"
            )
            path_to_model = model_info["path"]
    ...
    def train(
        ...
        self.trainer = pl.Trainer(
            max_epochs=self.epochs, gpus=self.gpus, callbacks=[checkpoint_callback]
        )
