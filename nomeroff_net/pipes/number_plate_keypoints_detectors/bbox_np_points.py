...
class NpPointsCraft(object):
    ...
    def load(
        self, mtl_model_path: str = "latest", refiner_model_path: str = "latest"
    ) -> None:
        """
    ...
    def load_model(
        ...
        refiner_model: str = os.path.join(
        ...
        if is_refine:
            ...
            self.refine_net.load_state_dict(
                copy_state_dict(torch.load(refiner_model, map_location="cpu"))
            )
            if is_cuda:
    ...
    def detect(
        ...
        return self.postprocess(
            model_outputs, quality_profile, text_threshold, link_threshold, low_text
        )
    ...
    def preprocess(
        self, inputs: Any, canvas_size: int = 300, mag_ratio: float = 1.0, **_
    ) -> Any:
        res = []
        for image_id, (image, target_boxes) in enumerate(inputs):
            for target_box in target_boxes:
                ...
                x, ratio_h, ratio_w = self.preprocessing_craft(
                    image_part, canvas_size, mag_ratio
                )
                res.append(
                    [
                        x,
    ...
    def postprocess(
        ...
        **_
        ...
        for (
            ...
            dimensions = [
                {"dx": distance(poly[0], poly[1]), "dy": distance(poly[1], poly[2])}
                for poly in bboxes
            ]
            ...
            if len(np_bboxes_idx) > 1:
                started_boxes = np.concatenate(
                    [bboxes[i] for i in np_bboxes_idx], axis=0
                )
                target_points = minimum_bounding_rectangle(
                    np.concatenate(multiline_rects, axis=0)
                )
                np_bboxes_idx, garbage_bboxes_idx, probably_count_lines = filter_boxes(
                    bboxes, dimensions, target_points, np_bboxes_idx
                )
                filtred_boxes = np.concatenate(
                    [bboxes[i] for i in np_bboxes_idx], axis=0
                )
            ...
            if len(np_bboxes_idx) > 0:
                ...
                target_points = addopt_rect_to_bbox(
                    target_points, image_shape, 7, 12, 0, 12
                )
            ...
            if len(propably_points):
                target_points_variants = make_rect_variants(
                    propably_points, quality_profile
                )
                if len(target_points_variants):
                    ...
                    if len(target_points_variants) > 1:
                        img_parts = [
                            get_cv_zone_rgb(image, reshape_points(rect, 1))
                            for rect in target_points_variants
                        ]
                        idx = detect_best_perspective(
                            normalize_perspective_images(img_parts)
                        )
                    ...
                    if in_zone_only:
                        for i in range(len(points)):
                            ...
                            points[i][0] = (
                                x0 + w0 if points[i][0] > x0 + w0 else points[i][0]
                            )
                            points[i][1] = (
                                y0 + h0 if points[i][1] > y0 + h0 else points[i][1]
                            )
                    all_points.append(points)
                else:
                    all_points.append(
                        [[x0, y0 + h0], [x0, y0], [x0 + w0, y0], [x0 + w0, y0 + h0]]
                    )
        ...
        for point, mline_box, image_id in zip(
            all_points, all_mline_boxes, all_image_ids
        ):
