...
lib = ctypes.CDLL(
    os.path.join(os.path.abspath(os.path.dirname(__file__)), "./build/libfast_boxes.so")
)
...
def find_char_boxes(markers, num_classes):
    ...
    c_find_char_boxes(
        markers, markers.shape[1], markers.shape[0], out_boxes.shape[0], out_boxes
    )
    ...
    for idx in range(2, num_classes + 1):
        ...
        w, h = r - l, b - t
        box = np.array(
            [[l, t], [l + w, t], [l + w, t + h], [l, t + h]], dtype=np.float32
        )
...
def find_word_boxes(
    textmap, labels, nLabels, stats, text_threshold, fast_mode=False, rotated_box=True
):
    """
    ...
    stat_left = np.array(
        [stats[k, cv2.CC_STAT_LEFT] for k in range(nLabels)], dtype=np.float32
    )
    stat_top = np.array(
        [stats[k, cv2.CC_STAT_TOP] for k in range(nLabels)], dtype=np.float32
    )
    stat_width = np.array(
        [stats[k, cv2.CC_STAT_WIDTH] for k in range(nLabels)], dtype=np.float32
    )
    stat_height = np.array(
        [stats[k, cv2.CC_STAT_HEIGHT] for k in range(nLabels)], dtype=np.float32
    )
    stat_area = np.array(
        [stats[k, cv2.CC_STAT_AREA] for k in range(nLabels)], dtype=np.float32
    )

    num_boxes = c_find_word_boxes(
        textmap,
        textmap.shape[1],
        textmap.shape[0],
        labels,
    ...
    out_boxes = [
        np.array(out_boxes[x * 8 : x * 8 + 8]).reshape(4, 2) for x in range(num_boxes)
    ]
